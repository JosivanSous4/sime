 ...
 * 
 */
public class Agent implements Trackable, Comparable<Agent>, NodeConnectionListener, GroupMembershipListener {
...
//			informaCoordenadas(Global.coordenadas.getLat(), Global.coordenadas.getLon());
//		}
//		catch (Exception e) {
//			e.printStackTrace();
//		}
	 ...
	 * Informa as coordenadas ao seletor.
	 */
	public void informaCoordenadas(double lat, double lng)
	{
		Global.coordenadas.setLat(lat);
		Global.coordenadas.setLon(lng);
		// informa as coordenadas ao seletor
		try {
			ApplicationMessage mensagem = new ApplicationMessage();
			Usuario user = new Usuario(this.username, Global.coordenadas.getLat(), Global.coordenadas.getLon());
			Gson gson = new Gson();
			String json = gson.toJson(user);
			System.out.println("ClassUser" + json);
